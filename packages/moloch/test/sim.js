...
contract('Moloch', accounts => {
  let snapshotId = 0, proposalIndex = 0, numAccounts = 35, applicants = []
  ...
  it('submit proposal', async () => {
    ...
    for (let x=1; x <= applicants.length; x++) {
      ...
      let tx = await moloch.submitProposal(accounts[x], applicant.tokenTribute, applicant.sharesRequested, "proposal number " + String(proposalIndex) + "from " + accounts[x])
